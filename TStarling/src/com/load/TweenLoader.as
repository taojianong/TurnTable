package com.load {
	...
	public class TweenLoader extends TEventDispatcher{
		...
		public static function getSWFComponent( nameOrUrl:String , link:String = "" ):*{
			...
			var app:ApplicationDomain = TweenLoader.getApp( nameOrUrl );
		...
		public function loadList( list:Array , complete:Function = null ):void {
			...
			function completeHandler1(event:LoaderEvent):void {
				...
				if ( _this.hasEventListener( TweenLoaderEvent.LOAD_SINGLE_COMPLETE ) ) {
					_this.dispatchEvent( new TweenLoaderEvent( TweenLoaderEvent.LOAD_SINGLE_COMPLETE , { name:event.target.name , "progress":1 } ) );
				}
			}
			...
			function errorHandler1(event:LoaderEvent):void {
				...
				if ( _this.hasEventListener( TweenLoaderEvent.LOAD_SINGLE_ERROR ) ) {
					_this.dispatchEvent( new TweenLoaderEvent( TweenLoaderEvent.LOAD_SINGLE_ERROR , { name:event.target.name ,"error":event.text } ) );
				}
			}
		...
		private function completeHandler(event:LoaderEvent):void {
			...
			this.cacheSource( event.target.name );
		}
		...
		//缓存资源
		private function cacheSource( id:String ):void {
			...
			var cont:* = TweenLoader.getData( res.id );
