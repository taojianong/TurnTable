package com.log {
	...
	public class TLog {
		...
		private static const LOG_NAME:Object = {			 
			...
			7: { "text":"【 战斗 】" , "color":"#00FF00" }			
		}
		
		private static var _unshowList:Array = [ ]; //不显示LOG类型,见上面类型
		public static function set unshowList( value:Array ):void {
		 ...
		 * @param ...args 日志类型数据
		 */
		public static function addunshowLog( ...args ):void {
			
			_unshowList = _unshowList.concat( args );
		}
		 ...
		 * @param ...args 日志类型数据
		 */
		public static function removeunshowLog( ...args ):void {
			
			for each( var type:int in _unshowList ) {
				for each( var arg:int in args ) {
					if ( arg == type ) {
						_unshowList.splice( _unshowList.indexOf( type ) , 1 );
						break;
					}
				}
			}
		...
		public static const LOG_FIGHT:uint	= 7 ; //战斗类型
		
		public static var showTypes:Array    = [ LOG_NOMAL , LOG_DEBUG , LOG_WARN , LOG_ERROR , LOG_LOAD , LOG_ALL , LOG_SOCKET , LOG_FIGHT ]; //要显示的所有类型.
		...
		private static function showLog( info:String , type:uint , id:int ):void {
			...
			if ( isShowLog ) { //判断是否显示日志
				...
				//如果该类型不在[不显示列表]里面,则可显示
				if ( _unshowList.indexOf( type ) != -1 ) {
					return;
				}
