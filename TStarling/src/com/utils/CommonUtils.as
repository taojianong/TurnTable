package com.utils {
	...
	public class CommonUtils {
		 ...
		 *
		 */
		public static function getColorHtmlText(str:*, color:String = "#00ff00" , fontSize:int = 14 ):String {
		...
		/**
		 * 将字符串格式为 {[#ff0000]此格式字符串} 解析为 <font color='#ff0000'>此格式字符串</font>,为HtmlText格式字符串
		 * @param  str
		 * @param  type 0为默认模式，1为多个颜色格式
		 * @return
		 */
		public static function analysisHtmlString( str:String , type:int = 0 , fontSize:int = 16 ):String {
			...
			if ( type == 0 ) {
				str = str.replace(/\{\[#.*\][^\}]+\}/g, function():String {
					temp = arguments[0];
					temp = temp.replace(/\{|\}/g, "");
					temp = temp.replace(/\[#.*\]/, function():String {
						
							color = arguments[0];
							color = color.replace(/\[|\]/g, "");
							return "";
						});
					temp = CommonUtils.getColorHtmlText( temp , color , fontSize );
					return temp;
				});
			}
			else if( type == 1 ){
				str = str.replace(/\{\[#[0-9a-zA-Z]+\][^\}]+\}/g, function():String {
					temp = arguments[0];
					temp = temp.replace(/\{|\}/g, "");
					temp = temp.replace(/\[#.*\]/, function():String {
		...
		/**
		 * 将字符串解析为带事件的htmlText文本
		 * @param	str  原字符串
		 * @param	info 点击信息
		 * @return
		 */
		public static function analysisEventHtmlString( str:String , info:String = "click" , fontSize:int=16):String {
		    ...
		    var ind:int;
		    var color:String;
		    str = str.replace(/\{link\[#.*\][^\}]+\}/g, function():String {
				temp = arguments[0];
				...
				return getHtmlEventString( temp , info , color , fontSize );			
			});	
		 ...
		 * @return
		 */
		public static function getHtmlEventString( value:String , info:String = "click" , color:String = "#00ff00" , size:int = 12 ):String {
			
			return "<font color='" + color + "' size=\'" + size+ "'><a href='event:" + info +"'><u>" + value + "</u></a></font>";	
		}
		
		/**
		 * 把str解析成xml格式，比如str：#cccccc111#/
		 * @param	str
		 */
		public static function getHtmlString(str:String, textFormat:TextFormat = null):String {
			...
			while (str.indexOf("$/") != -1) {
				...
				index = str.indexOf("$");
				color = str.substr(index, 7);
				str = str.replace(color, "<font color='#" + color.replace("$", "") + "'>");
				index = str.indexOf("$/");
			...
			if (textFormat != null) {
				...
				str = "<textformat leading='10'><font face='" + textFormat.font + "'" + " color='#" + textFormat.color.toString(16) + "'" + " size='" + textFormat.size + "'>" + str + "</font></<textformat>";
				
				str = textFormat.bold ? "<b>" + str + "</b>" : str;
			}
		 ...
		 * @return
		 */
		public static function getObjectKeys(obj:Object):int {
		 ...
		 * @param	j
		 * @param	range
		 * @param   type 0 为取对应范围数组的值，1为取对应范围的节点坐标
		 * @return
		 */
		public static function getDyadicArrayRange( arr:Array , i:int , j:int , range:int = 0 , type:int = 0 ):Array {
			...
			if (arr[i] != null) {
				...
				if ( val != null) {
					...
					if ( range == 0 ) {						
						type == 0 ? a.push( val ) : a.push( new Point( i , j ) );											
						return a;
					}
					...
					for (m = minI; m <= maxI; m++) {
						...
						for (n = minJ; n <= maxJ; n++) {
							...
							val = arr[m][n];
							//a.push(arr[m][n]);
							type == 0 ? a.push( val ) : a.push( new Point( m , n ) );	
						}
					}
				}
			}
